...
public class ProductDAOImpl implements ProductDAO {
	...
	public int insert(ProductVO pvo) {
		...
		try {
			...
			pst.setString(1, pvo.getPname()); // param index, value
			pst.setInt(2, pvo.getPrice());
			pst.setString(3, pvo.getMade_by());
			return pst.executeUpdate(); //query update
	...
	public List<ProductVO> selectList() {
		...
		try {
			...
			while(rs.next()) {
				list.add(new ProductVO(rs.getInt("pno"), rs.getString("pname"), rs.getString("reg_date")));
			}
			return list;
		} catch (SQLException e) {
			e.printStackTrace();
